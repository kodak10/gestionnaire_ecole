...
<div class="container mt-4" style="display:flex; gap:20px;">
    ...
    <div style="flex:3; background:#fff; padding:20px; border-radius:8px; box-shadow:0 2px 8px rgba(0,0,0,0.1);">
        <h2 class="mb-3">Modifier le modèle SMS</h2>
        <form action="{{ route('templates.update', $template->id) }}" method="POST" id="template-form">
            @csrf
            @method('PUT')
            
            <div class="form-group">
                <label for="nom">Nom du modèle</label>
                <input type="text" class="form-control" id="nom" name="nom" value="{{ old('nom', $template->nom) }}" required>
            </div>
            
            <div class="form-group">
                <label for="type">Type de document</label>
                <select class="form-control" id="type" name="type" required>
                    <option value="recu_paiement" {{ $template->type == 'recu_paiement' ? 'selected' : '' }}>Reçu de paiement</option>
                    <option value="relance" {{ $template->type == 'relance' ? 'selected' : '' }}>Relance</option>
                    <option value="information" {{ $template->type == 'information' ? 'selected' : '' }}>Information</option>
            ...
            <div class="form-group">
                ...
                <textarea name="content" id="editor" 
                    ...
                    placeholder="Saisissez votre message ici...">{{ old('content', $template->content) }}</textarea>
            </div>
    ...
    <div style="flex:1; background:#f8f9fa; padding:20px; border-radius:8px; box-shadow:0 2px 8px rgba(0,0,0,0.05);">
        ...
        @if($template->type == 'recu_paiement')
        ...
        @elseif($template->type == 'information')
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    document.getElementById('preview-btn').addEventListener('click', function() {
        ...
        const testData = {
            ...
            'DATE': '15/01/2026',
            'DATE_FR': '15 janvier 2026',
            'ANNEE': '2025-2026',
            'MONTANT_DU': '25 000',
            'MONTANT_DU_LETTRES': 'Vingt-cinq mille',
            'DATE_ECHEANCE': '31/01/2026',
            'RETARD': '5',
